...
const RenderVirtualizedTableBody =
    ...
    (provided: DroppableProvided): ReactElement => {
        ...
        const { selectedCacheById, refs, highlightedRowRef, state, initializeRef, deleteRef } =
            instance;

        const getInnerRef = (innerRef: DroppableProvided['innerRef']) => (node: HTMLDivElement) => {
            innerRef(node);
        ...
        return (
            <div ref={getInnerRef(innerRef)} {...getTableBodyProps()} {...droppableProps}>
                <InfiniteLoader
                    itemCount={itemCount}
                    loadMoreItems={() => {
                        onLoadPage?.();
